...
{
    // speed y
    if(g_Config.m_SvAnnouncementSpeed >= 0)
    {
        CPlayer *pPlayer = m_apPlayers[g_Config.m_SvAnnouncementSpeed];
        if(pPlayer && pPlayer->GetCharacter())
        {
            CCharacter *pChr = pPlayer->GetCharacter();
            float VelspeedY = pChr->GetCore().m_Vel.y * 50 / 32;
			float VelspeedX = pChr->GetCore().m_Vel.x * 50 / 32;

            if (VelspeedY != 0 || (VelspeedX < -0.02 || VelspeedX > 0.02)) {
                char aBuf[256];
                str_format(aBuf, sizeof(aBuf), "X: %.2f | Y: %.2f", VelspeedX, VelspeedY);
...
void CGameContext::RegisterDDRaceCommands()
{
	...
	Console()->Register("tele_all", "?i[id]", CFGFLAG_SERVER | CMDFLAG_TEST, ConTeleportAll, this, "Teleports all players to player i (or you to where you look at)");
	...
	Console()->Register("setvel", "i[clientid] f[x] f[y]", CFGFLAG_SERVER | CMDFLAG_TEST, ConSetVel, this, "Set velocity of tee");
